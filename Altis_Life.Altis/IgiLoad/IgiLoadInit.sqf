...
{
	if (((typeOf _x) in (IL_Supported_Vehicles_All)) && (playerSide == west || playerSide == independent)) then
	{
...
while {true} do 
{
	...
	{
		if ((((typeOf _x) in (IL_Supported_Vehicles_All)) && !(_x in IL_Veh_Array)) && (playerSide == west || playerSide == independent))  then
		{
			IL_Veh_Array = IL_Veh_Array + [_x];
			_null = [_x] execVM "IgiLoad\IgiLoad.sqf";
			waitUntil {scriptDone _null};
